...
function seedAvengersData() {
  ...
  if (fs.existsSync(cronSrc) && !fs.existsSync(cronDest)) {
    fs.mkdirSync(cronDir, { recursive: true });
    fs.copyFileSync(cronSrc, cronDest);
    console.log("[avengers] Seeded cron jobs");
  }
}

function patchAvengersConfig() {
  const agentsSeedPath = path.join(SEED_DIR, "agents.json");
  if (!fs.existsSync(agentsSeedPath)) return "";
  ...
  try {
    ...
    const avengers = JSON.parse(fs.readFileSync(agentsSeedPath, "utf8"));

    // Overlay Avengers config onto onboarding-generated config
    for (const [key, value] of Object.entries(avengers)) {
      current[key] = value;
    }
...
app.post("/setup/api/run", requireSetupAuth, async (req, res) => {
  ...
  if (ok) {
    ...
    // Patch Avengers 9-agent config into the onboarding-generated config.
    extra += patchAvengersConfig();
